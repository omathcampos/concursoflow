...
## Estado do projeto
Fases 0–11 concluídas — roadmap original (`README.md`) inteiro concluído. `v1.3.0` é o release mais recente em `master`, no ar na Vercel (produção = `master`, previews = PRs) — https://concursoflow-alpha.vercel.app. `develop` e `master` estão sincronizados (backmerge feito via PR). `src/middleware.ts` foi renomeado para `src/proxy.ts` (convenção do Next.js 16 — o export também virou `proxy`, não `middleware`).
