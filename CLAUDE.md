...
## Visão em 3 linhas
ConcursoFlow é um app de cronograma de estudos para concursos públicos (calendário, ciclo de estudos, sessões, revisões espaçadas, dashboard, notificações, modo foco, exportação de calendário). Stack: Next.js 16 (App Router, Turbopack) + TypeScript strict + Tailwind v4 + shadcn/ui (Base UI) + Zustand (local-first e cache do Supabase) + Supabase (banco real desde a fase 7, com pg_cron + Edge Functions desde a fase 10). Estado atual: **v1.6.0 publicado em produção (Vercel), `develop` e `master` sincronizados; roadmap original + Fase 12 (exportar calendário) concluídos — Fase 13 (documentação) é o próximo prompt sincronizado, ainda não iniciado.**
...
## Estado do projeto
Fases 0–12 concluídas — roadmap original (`README.md`) inteiro concluído + Fase 12 (exportar calendário). `v1.6.0` é o release mais recente em `master`, no ar na Vercel (produção = `master`, previews = PRs) — https://concursoflow-alpha.vercel.app. `develop` e `master` estão sincronizados (backmerge feito via PR). `src/middleware.ts` foi renomeado para `src/proxy.ts` (convenção do Next.js 16 — o export também virou `proxy`, não `middleware`).

Fase 12 (Exportar calendário) implementada: feed iCal (.ics) por assinatura via **Edge Function pública** `calendar-feed` (não um Next.js route handler — decisão deliberada: evita depender de `SUPABASE_SERVICE_ROLE_KEY` como env var do Vercel, que eu não tenho como obter/verificar via as tools disponíveis; Edge Functions já recebem esse secret automaticamente do Supabase). Identificado por `profiles.calendar_feed_token` (uuid regenerável, nunca pelo `auth.uid()`), aceita o token via `?token=` ou como último segmento do path (`/calendar-feed/<token>.ics`). `src/lib/domain/ical.ts` (TDD, validado contra `node-ical` nos testes) monta o `.ics` — decisão de não usar `VTIMEZONE`: todo horário vai em UTC (sufixo `Z`), já que os timestamps do banco já são instantes UTC e todo cliente de calendário converte pro fuso do dispositivo sozinho; evita ter que acertar regras de transição de fuso na mão. Lógica duplicada em `supabase/functions/_shared/ical.ts` (mesmo padrão de `content.ts`/`notifications.ts` — Deno não importa `src/lib/domain`). Botão "Baixar .ics" do período visível funciona 100% no client (sem backend), inclusive em modo `local`.

v1.3.0 corrigiu bugs reportados em produção logo após o v1.2.0 (Fases 10/11): revalidação por foco da janela virou silenciosa (dois gatilhos independentes: o listener de `focus` e o supabase-js reemitindo `SIGNED_IN` na revalidação de sessão — ambos passavam por `status:"loading"` e derrubavam a árvore de children do `DataProvider`, causando o "reload" visual a cada alt-tab); CORS nas Edge Functions `daily`/`weekly`/`overdue`/`unsubscribe` (preflight `OPTIONS` agora tratado por `supabase/functions/_shared/cors.ts` antes de qualquer auth); `TELEGRAM_BOT_TOKEN` corrigido (secret estava com valor inválido); erros reais no teste de notificação (ex.: restrição de sandbox do Resend) agora aparecem legíveis no toast em vez de um 500 genérico sem corpo.
...
- Teste manual real dos 3 envios (email + Telegram) ainda pendente — precisa de um usuário logado clicando em "enviar agora (teste)" na página de Perfil; não é automatizável em CI. CORS e token do Telegram já corrigidos em produção (v1.3.0); Resend segue em modo sandbox (só envia para o email verificado da própria conta) até um domínio ser verificado.
