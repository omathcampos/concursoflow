// Relatório semanal. Cron (diário ~23h UTC, a function confere se é domingo
// no fuso do usuário) ou teste manual (JWT do próprio usuário + ?test=true).
import { buildWeeklyContent, getLocalBounds } from "../_shared/build-content.ts";
import { renderWeeklyEmailHtml, renderWeeklyEmailText, renderWeeklyTelegramHtml, shouldSendWeekly } from "../_shared/content.ts";
...
async function processUser(admin: ReturnType<typeof getAdminClient>, prefs: NotificationPrefsRow, isTest: boolean, now: Date) {
  ...
  if (!isTest && !bounds.is_sunday) return { skipped: "not_sunday" };

  const content = await buildWeeklyContent(admin, prefs.user_id, prefs.timezone, bounds);
  if (!isTest && !shouldSendWeekly(content)) return { skipped: "empty" };

  const { data: profile } = await admin.from("profiles").select("display_name").eq("id", prefs.user_id).single();
  const displayName = profile?.display_name ?? null;
  const unsubscribeUrl = `${SITE_URL}/unsubscribe?token=${prefs.unsubscribe_token}&type=weekly`;
...
Deno.serve(async (req: Request) => {
  ...
  const { data: allPrefs } = await admin.from("notification_prefs").select("*").eq("weekly_enabled", true);
  const results = [];
