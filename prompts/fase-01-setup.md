...
6. Shell de layout: sidebar fixa (desktop) / drawer (mobile) com links: Dashboard (/), Calendário, Ciclo, Matérias, Sessões, Revisões, Anotações — cada um com ícone lucide. Header com nome do app, toggle de tema e espaço reservado para countdown da prova.
7. Criar as 7 páginas como placeholders estilizados (título + card "em construção" com descrição do que virá).
...
- [ ] Navegação entre as 7 páginas funciona, link ativo destacado na sidebar.
- [ ] Sidebar colapsa em drawer no mobile (< 768px).
- [ ] `types.ts` compila com todos os modelos (Subject, Topic, Cycle, CycleEntry, Block, Session, Review, Annotation) e enums.
